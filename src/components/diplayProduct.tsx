...
export function DisplayProduct() {
  const { loadingProducts, setLoadingProducts, setOneProduct, oneProductMode,setOneProductMode } = useStore();
  ...
  return (
    <div className="w-11/12 p-2 grid grid-cols-1 gap-7 bg-slate-50 rounded-md ">
      {loadingProducts ? (
        <>
          <ProductSkeleton />
          <ProductSkeleton />
        ...
        <>
          { productsData.map(({ name, inventory_count, description, price, id, category, image_logo }) => (
    ...
    <div className="w-full px-4">
      ...
      {/* Stock banner  */}
        <div className="text-sm font-medium text-green-600 bg-green-100 px-2 py-1 rounded-full"> 
           <span className="font-semibold">{inventory_count}</span> in stock </div>
      </div>
